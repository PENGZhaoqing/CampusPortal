class AccessController < ApplicationController
  ...
  def associate
    ...
    redirect_to cooperator_add_users_path(@application, @user),
  ...
  def disassociate
    ...
    redirect_to cooperator_list_users_path(@application, @user),
  ...
  def update
    ...
    if @access.update_attributes(node: node, path: path)
      ...
      redirect_to cooperator_edit_user_access_path(@user, @application), flash: {success: flash}
    else
      flash = 'Fail to update changes, please try it again '
      redirect_to cooperator_edit_user_access_path(@user, @application), flash: {danger: flash}
    end
  end
  ...
  def login_as_admin_or_cooperator
    if current_admin.nil? &&current_cooperator.nil?
  ...
  def set_access
    ...
    @access=@user.resource.accesses.find_by(app_id: params[:id])
  end
